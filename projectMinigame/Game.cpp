...
bool Game::Update()
{
	...
	if ((toggle_enemies == true && idx_Enemy < (MAX_ENEMIES - 1)) && (Enemy_delay == 0)) {
		int val1 = rand() % 2, val2 = rand() % 2, val3 = rand() % WINDOW_WIDTH, val4 = rand() % WINDOW_HEIGHT, x = 0, y = 0;
			...
			if (enemyType == 11) {
				Enemy[idx_Enemy].Init(x, y, 64, 80, 1, 400, (Player.GetX() - x) / sqrt(pow(Player.GetY() - y, 2) + pow(Player.GetX() - x, 2)), (Player.GetY() - y) / sqrt(pow(Player.GetY() - y, 2) + pow(Player.GetX() - x, 2)), enemyType);
